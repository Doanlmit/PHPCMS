...
class My_nested {
    ...
    public function check_empty($table = ''){
        ...
        if ($count == 0) {
            ...
            $back = $this->CI->my_string->php_redirect(CMS_BASE_URL . 'backend/'.current(explode('_', $table)).'/add'.end(explode('_', $table)));
        }   
    }
     //clip 14
    ...
    //Phương thúc data của Hải
    public function data_Root($table = '', $param = NULL) {
        $_lang = $this->CI->session->userdata('_lang');
        if ($param == NULL) {
            $data = $this->CI->db->from($table)->order_by('lft asc')->get()->result_array();
        } else {
            $data = $this->CI->db->from($table)->where($param)->or_where(array('lang'=> 'all'))->order_by('lft asc')->get()->result_array();
        }
        return $data;
    }
     //clip 14
    ...
    public function arr($table = '') {
        $_lang = $this->CI->session->userdata('_lang');
        return $this->CI->db->select('id, title, parentid, level, orders')->from($table)->where(array('lang'=> $_lang))->or_where(array('lang' => 'all'))->order_by('orders asc, id asc')->get()->result_array();
    }
     //clip 14
    //Chi tiết
    public function get($table = '',  $param = NULL) {
        return $this->CI->db->select('id, title, parentid, level, orders')->from($table)->where($param)->get()->row_array();
    }
     //clip 14
    //Đệ quy
    ...
    public function level($table = '') {
        $data = $this->CI->my_nested->recursive(0, $this->CI->my_nested->arr($table));
        if (isset($data) && count($data)) {
            ...
            foreach ($data as $key => $val) {
                ...
                if ($val['parentid'] == 0) {
                    $level = 0;
                }//Nếu không phải là node Root thì lấy level của cấp cha đó + thêm 1
                else if($val['parentid'] > 0){
                    $parent = $this->CI->my_nested->get($table, array('id'=>$val['parentid']));
    ...
    public function set($table = ''){
        $this->CI->my_nested->level($table);
        $this->CI->my_nested->lefrgt($table);
    }
    //Phương thức kiểm tra danh mục cha 
    public function check_parentid($table = '', $parentid = 0, $catid = 0) {
        ...
        if (isset($data) && count($data)) {
            $children = $this->CI->db->select('id')->from($table)->where(array('lang'=>$_lang))->where(array('lft >' => $data['lft'], 'lft < ' => $data['rgt']))->get()->result_array();
            ...
            $this->CI->form_validation->set_massage('_parentid', 'Danh mục cha không tồn tại.');
            return FALSE;
        }
        return TRUE;
    }
